...
function mgr.calculateVariableSettings(settings)
  for varNum = 1, 9, 1 do
    ...
    if settings[entityName] ~= nil and settings[entityName].name then
      ...
      if furnaceResult then
        for furnaceKey, furnaceValue in pairs(furnaceResult) do
          if #furnaceValue.ingredients == 1 and furnaceValue.main_product then
            settings[furnaceName] = { name = furnaceValue.main_product.name, type = furnaceValue.main_product.type }
            break
          end
        end
      end
