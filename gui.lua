...
script.on_event('bv-confirm', function(event)
  if global.players and global.players[event.player_index] then
    global.players[event.player_index].confirmTick = event.tick;
  end
end)
...
function mygui.gui_closed(event)
  ...
  if (event.name == defines.events.on_gui_closed and event.tick == global.players[event.player_index].confirmTick) then
    mygui.confirm(event)
